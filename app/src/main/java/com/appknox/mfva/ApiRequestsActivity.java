...
public class ApiRequestsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final TextView textViewLogs = findViewById(R.id.textview_api_requests);
        final FloatingActionButton buttonStartApiRequests = findViewById(R.id.button_start_api_requests);

        buttonStartApiRequests.setOnClickListener(v -> new Thread(() -> {
            ...
            Request request1 = new Request.Builder()
                    ...
                    .build();
            try {
                textViewLogs.post(() -> textViewLogs.append("Authenticating...\n"));
                String response = Objects.requireNonNull(client.newCall(request1).execute().body()).string();
            ...
            if(token == null) {
                textViewLogs.post(() -> textViewLogs.append("Unable to authenticate!\n"));
                return;
            }
            ...
            Request request2 = new Request.Builder()
                    ...
                    .build();
            try {
                textViewLogs.post(() -> textViewLogs.append("Fetching user ID...\n"));
                JSONObject json2 = new JSONObject(Objects.requireNonNull(client.newCall(request2).execute().body()).string());
                final String id2 = json2.getJSONObject("response").getJSONObject("user").getString("id");
                textViewLogs.post(() -> textViewLogs.append("  userid:" + id2 + "\n"));
            } catch (IOException|JSONException e) {
                Snackbar.make(v, e.toString(), Snackbar.LENGTH_SHORT).show();
            }
            ...
            Request request5 = new Request.Builder()
                    ...
                    .build();
            try {
                textViewLogs.post(() -> textViewLogs.append("Adding new user...\n"));
            ...
            try {
                ...
                textViewLogs.post(() -> textViewLogs.append("Getting server uptime...\n"));
                ...
                JSONObject json4 = new JSONObject(Objects.requireNonNull(client.newCall(request4).execute().body()).string());
                final String uptime = json4.getJSONObject("response").getString("Output");
                textViewLogs.post(() -> textViewLogs.append("  system up since:" + uptime.trim() + "\n"));
            } catch (IOException|JSONException e) {
                Snackbar.make(v, e.toString(), Snackbar.LENGTH_SHORT).show();
            }

            textViewLogs.post(() -> textViewLogs.append("Done!\n"));
        }).start());
    }
}
